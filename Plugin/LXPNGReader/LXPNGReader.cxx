...
vtkStandardNewMacro(LXPNGReader);
...
class LXPNGReader::vtkInternals
{
public:
  ...
  void InitLibPngInput(LXPNGReader* self, png_structp pngPtr, MemoryBufferStream* stream, FILE* fp)
  {
...
//----------------------------------------------------------------------------
void LXPNGReader::ExecuteInformation()
{
...
//----------------------------------------------------------------------------
template <class OT>
void LXPNGReader::LXPNGReaderUpdate2(
  OT *outPtr, int *outExt, vtkIdType *outInc, long pixSize)
{
  LXPNGReader::vtkInternals* impl = this->Internals;
  unsigned int ui;
  int i;
  FILE* fp = nullptr;
  MemoryBufferStream stream;
...
template <class OT>
void LXPNGReader::LXPNGReaderUpdate(
  vtkImageData *data, OT *outPtr)
{
  ...
  for (idx2 = outExtent[4]; idx2 <= outExtent[5]; ++idx2)
  {
    ...
    // read in a PNG file
    this->LXPNGReaderUpdate2(outPtr2, outExtent, outIncr, pixSize);
...
void LXPNGReader::ExecuteDataWithInformation(vtkDataObject *output,
                                              vtkInformation *outInfo)
{
  ...
  {
    vtkTemplateMacro(this->LXPNGReaderUpdate(data, (VTK_TT *)(outPtr)));
    default:
...
//----------------------------------------------------------------------------
int LXPNGReader::CanReadFile(const char* fname)
{
...
//----------------------------------------------------------------------------
void LXPNGReader::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void LXPNGReader::GetTextChunks(const char* key, int beginEndIndex[2])
{
  this->Internals->GetTextChunks(key, beginEndIndex);
}

//----------------------------------------------------------------------------
const char* LXPNGReader::GetTextKey(int index)
{
  return this->Internals->TextKeyValue[index].first.c_str();
}

//----------------------------------------------------------------------------
const char* LXPNGReader::GetTextValue(int index)
{
  return this->Internals->TextKeyValue[index].second.c_str();
}

size_t LXPNGReader::GetNumberOfTextChunks()
{
